stat_NEP_GPP_Photo <- function(dat) {
  ...
  for (i in id){
    fit1 <- try(nlsLM(values~offset + A*(GPP^B)*(exp(k*GPP)), data = dat[dat$Site_ID != i,], 
                      start = list(A = 3.852e-09, B =2.667e+00, k =-1.278e-03, offset= -1.5), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit5 <- try(nlsLM(values~offset + A*(1+((B*((GPP/C)^D)-1)/(exp(GPP/C)))), data = dat[dat$Site_ID != i,], 
                      start = list(A = 0.188, B =  -4.871, C =  365.122, D=  -0.393, offset= -1.5), control = list(maxiter = 500)), silent=TRUE); 
