stat_NEP_Photo <- function(dat) {
  ...
  for (i in id){
    fit1 <- try(nlsLM(values~A*(GPP^B)*(exp(k*GPP)), data = dat[dat$Site_ID != i,], 
                      start = list(A = 1.943e-12, B =4.902e+00, k =-2.154e-03), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit5 <- try(nlsLM(values~A*(1+((B*((Stand_Age/C)^D)-1)/(exp(Stand_Age/C)))), data = dat[dat$Site_ID != i,], 
                      start = list(A = 213.0138, B =  -6.2368, C = 2.8525, D= 0.6851), control = list(maxiter = 500)), silent=TRUE); 
