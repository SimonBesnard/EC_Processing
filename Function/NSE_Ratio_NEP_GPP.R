stat_Ratio_NEP_GPP <- function(dat) {
  ...
  Besnard<-c()
  for (i in id){
    fit1 <- try(nlsLM(values~offset + A*(Stand_Age^B)*(exp(k*Stand_Age)), data = dat[dat$Site_ID != i,], 
                    start = list(A =-1.1422, B = 0.7842, k = -0.4142, offset= -1.5), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit4 <- try(nlsLM(values~offset + A*(1-exp(k*Stand_Age)), data = dat[dat$Site_ID != i,], 
