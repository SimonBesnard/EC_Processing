stat_Temp_GPP_ER <- function(dat) {
  ...
  for (i in id){
    fit1 <- try(nlsLM(values~A/(1+exp(B-C*Tair)), data=dat[dat$Site_ID != i,],
                      start = list(A= 34.50011, B=3.46889, C= 0.01089), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit2 <- try(nlsLM(values~A*Tair^2+B*Tair+C, data = dat[dat$Site_ID != i,], 
                      start = list(A= -0.001214, B=0.036857, C= 0.979401), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit3 <- try(nlsLM(values~A*Tair^3+B*Tair^2+C*Tair+D, data = dat[dat$Site_ID != i,], 
                      start = list(A=-1.553e-05, B=-7.161e-04, C= 3.382e-02, D=9.782e-01), control = list(maxiter = 500)), silent=TRUE); 
