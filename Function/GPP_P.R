stat_GPP_P <- function(dat) {
  ...
  for (i in id){
    fit1 <- try(nlsLM(values~A*(1-exp(-B*Annual_Preci)), data = dat[dat$Site_ID != i,],
                      start = list(A= 3.168e+03, B=6.259e-04), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit2 <- try(nlsLM(values~A*Annual_Preci^2+B*Annual_Preci+C, data = dat[dat$Site_ID != i,], 
                      start = list(A=-4.945e-05 , B=1.012e+00, C= 4.204e+02), control = list(maxiter = 500)), silent=TRUE);
    ...
    fit3 <- try(nlsLM(values~A*Annual_Preci^3+B*Annual_Preci^2+C*Annual_Preci+D, data = dat[dat$Site_ID != i,], 
                      start = list(A=2.078e-07, B=-9.925e-04, C= 2.106e+00, D=1.038e+02), control = list(maxiter = 500)), silent=TRUE); 
