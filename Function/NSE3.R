stat3 <- function(dat) {
  ...
  for (i in 1:id){
    fit1 <- try(nls(values~A*(Stand_Age^B)*(exp(k*Stand_Age)), data = dat[-i,], 
                    start = list(A = 1000, B = 0.170, k = -0.00295)), silent=TRUE);
  ...
  list(Gamma, Second_Poly, Third_Poly, Amiro)
  # c(NSE(Gamma, dat$values, na.rm=TRUE), NSE(Second_Poly, dat$values, na.rm=TRUE), NSE(Third_Poly, dat$values, na.rm=TRUE), NSE(Amiro, dat$values, na.rm=TRUE ))
}
