...
Fun_NEP<-nlsLM(values~offset + A*(1-exp(k*Stand_Age)), data = NEP, 
               start = list(A=192.93829, k=-0.08976, offset=-700), control = list(maxiter = 500))
...
gg1<-ggplot(predVals, aes(x, lower, upper)) +
  geom_point(data = subset(NEP, !is.na(MAP_CRU)), aes(x = Stand_Age, y = values, size=MAP_CRU, colour=GPP), inherit.aes = FALSE)+
  geom_point(data = subset(NEP, is.na(MAP_CRU)), aes(x = Stand_Age, y = values), colour='grey50', inherit.aes = FALSE)+
...
# Compute the best fit function
Fun_NEP<-nlsLM(values~offset + A*(1-exp(k*Stand_Age)), data = NEP_Mean_Site, 
               start = list(A=192.93829, k=-0.08976, offset=-700), control = list(maxiter = 500))
...
# Plot using ggplot
gg1<-ggplot(predVals, aes(x, lower, upper)) +
  geom_point(data = subset(NEP_Mean_Site, !is.na(MAP_CRU)),  aes(x = Stand_Age, y = values, size=MAP_CRU, colour=GPP), inherit.aes = FALSE)+
  geom_point(data = subset(NEP_Mean_Site, is.na(MAP_CRU)),  aes(x = Stand_Age, y = values), colour='grey50', inherit.aes = FALSE)+
  geom_line(aes(y = fit), colour="#666666", size=0.8)+
  geom_line(mapping = aes(y = upper), colour="#666666", lty = "dashed", size=0.8) +
  geom_line(mapping = aes(y = lower), colour="#666666", lty = "dashed", size=0.8)+
  scale_colour_gradient(low="#e0f3f8", high ="#d73027", space="Lab")+
