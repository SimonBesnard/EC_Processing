...
lm.NEP<-lm(values ~ (f_Tair + f_Age + f_GPP + SPI_CRU)^2, data=NEP)
...
for(id in unique(NEP$Site_ID)){
  ...
  lm.NEP<- lm(values ~f_Tair + f_Age + f_GPP + SPI_CRU + f_Tair:f_GPP + 
                f_Age:SPI_CRU, data=train.df)
...
# Perform stepwise regression
lm.NEP<-lm(values ~ (f_Tair + f_Age_GPP + f_GPP + SPI_CRU)^2, data=NEP)
step.NEP<- step(lm.NEP, direction = "backward")
print(step.NEP)
summary(step.NEP)
...
for(id in unique(NEP$Site_ID)){
  ...
  lm.NEP<- lm(values ~f_Tair + f_Age_GPP + f_GPP + SPI_CRU + 
                f_Tair:f_GPP + f_Age_GPP:f_GPP + f_Age_GPP:SPI_CRU, data=train.df)
  NEP.pred = predict(object = lm.NEP, newdata = test.df)
...
for(id in unique(NEP$Site_ID)){
  ...
  lm.NEP<- lm(values ~ f_Tair + f_Age_GPP + f_GPP + SPI_CRU + 
                f_Tair:f_Age_GPP + f_Tair:f_GPP + f_Tair:SPI_CRU + f_Age_GPP:f_GPP + 
                f_Age_GPP:SPI_CRU, data=train.df)
  NEP.pred = predict(object = lm.NEP, newdata = test.df)
...
gg2<- ggplot(NEP, aes(x=pred_Model2, y=values, colour=Stand_Age))+
  ...
  annotate("text", label = "RMSE = 186.04 gC.m-2.y-1", x = -390, y = 550, size =4)
...
gg3<- ggplot(NEP, aes(x=pred_Model3, y=values, colour=Stand_Age))+
  ...
  xlim(-700, 800)+
  ylim(-700, 800)+
  annotate("text", label = "R-squared = 0.46", x = -500, y = 700, size =4) +
  annotate("text", label = "RMSE = 183.8 gC.m-2.y-1", x = -390, y = 550, size =4)
...
for(id in unique(NEP$Site_ID)){
  train.df <- NEP[NEP$Site_ID != id,]
  test.df <- NEP[NEP$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "SPI_CRU")]
  lm.NEP<- lm(values ~ f_Tair + f_Age + f_GPP + SPI_CRU + f_Age:f_GPP + 
                f_Age:SPI_CRU + f_GPP:SPI_CRU, data=train.df)
  step.NEP<- step(lm.NEP, direction = "backward")
...
for(id in unique(NEP$Site_ID)){
  train.df <- NEP[NEP$Site_ID != id,]
  test.df <- NEP[NEP$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age_GPP", "f_GPP", "SPI_CRU")]
  lm.NEP<- lm(values ~f_Tair + f_Age_GPP + f_GPP + SPI_CRU + 
                f_Age_GPP:f_GPP + f_Age_GPP:SPI_CRU + f_GPP:SPI_CRU, data=train.df)
  step.NEP<- step(lm.NEP, direction = "backward")
  NEP.pred = predict(object = step.NEP, newdata = test.df)
  NEP$pred_Model2[NEP$Site_ID == id] <- NEP.pred
}
...
for(id in unique(NEP$Site_ID)){
  ...
  test.df <- NEP[NEP$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age_GPP", "f_GPP", "SPI_CRU")]
  lm.NEP<- lm(values ~ f_Tair + f_Age_GPP + f_GPP + SPI_CRU + 
                f_Tair:f_GPP + f_Age_GPP:f_GPP + f_Age_GPP:SPI_CRU + f_GPP:SPI_CRU, data=train.df)
  step.NEP<- step(lm.NEP, direction = "backward")
  NEP.pred = predict(object = step.NEP, newdata = test.df)
  NEP$pred_Model3[NEP$Site_ID == id] <- NEP.pred
}
...
gg1<- ggplot(NEP, aes(x=pred_Model1, y=values, colour=Stand_Age))+
  ...
  xlim(-700, 800)+
  ylim(-700, 800)+
  annotate("text", label = "R-squared = 0.47", x = -500, y = 700, size =4) +
  annotate("text", label = "RMSE = 186.2 gC.m-2.y-1",  x = -370, y = 550, size =4)
...
gg2<- ggplot(NEP, aes(x=pred_Model2, y=values, colour=Stand_Age))+
  ...
  xlim(-700, 800)+
  ylim(-700, 800)+ 
  annotate("text", label = "R-squared = 0.35", x = -500, y = 700, size =4) +
  annotate("text", label = "RMSE = 207.5 gC.m-2.y-1",  x = -370, y = 550, size =4)
...
gg3<- ggplot(NEP, aes(x=pred_Model3, y=values, colour=Stand_Age))+
  ...
  xlim(-700, 800)+
  ylim(-700, 800)+
  annotate("text", label = "R-squared = 0.29", x = -500, y = 700, size =4) +
  annotate("text", label = "RMSE = 225.1 gC.m-2.y-1",  x = -370, y = 550, size =4)
