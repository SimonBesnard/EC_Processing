...
# Stepwise regression
lm.NEP<-lm(values ~  (f_Age + f_GPP + f_Tair)^2, data=NEP)
step.NEP<- step(lm.NEP, direction = "backward")
summary(step.NEP)

# Compute Importance variable
bootswiss <- boot.relimp(values~ f_Age + f_GPP + SPEI + f_Tair + f_Age:f_Tair + 
                           f_GPP:f_Tair, 
                         data= NEP, 
                         b = 100,  
                         type = "lmg",
...
for(id in unique(NEP$Site_ID)){
  ...
  test.df <- NEP[NEP$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "SPEI")]
  lm.NEP<- lm(values ~ f_Age + f_GPP + SPEI + f_Tair + f_Age:f_Tair + 
...
# Stepwise regression
lm.NEP_Mean_Site<-lm(values ~ (f_P + f_Tair + f_Age + f_GPP)^2, data=NEP_Mean_Site)
step.NEP_Mean_Site<- step(lm.NEP_Mean_Site, direction = "backward")
print(step.NEP_Mean_Site)
summary(step.NEP_Mean_Site)

# Compute Importance variable
bootswiss <- boot.relimp(values~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair + 
                           f_P:f_Age + f_Tair:f_GPP, 
                         data= NEP_Mean_Site, 
                         b = 100,  
                         type = "lmg",
                         rank = TRUE, diff = TRUE, rela = T)
...
# Assess model performance
NEP_Mean_Site<- NEP_Mean_Site[c("Site_ID",  "Type_Flux", "Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "values")]
NEP_Mean_Site$prediction <- NA
for(id in unique(NEP_Mean_Site$Site_ID)){
  train.df <- NEP_Mean_Site[NEP_Mean_Site$Site_ID != id,]
  test.df <- NEP_Mean_Site[NEP_Mean_Site$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP")]
  lm.NEP_Mean_Site<- lm(values ~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair + 
                          f_P:f_Age + f_Tair:f_GPP, data=train.df)
  step.NEP_Mean_Site<- step(lm.NEP_Mean_Site, direction = "backward")
...
# Stepwise regression
lm.Ratio_GPP_Reco<-lm(values ~ (f_P + f_Tair + f_Age + f_GPP)^2, data=Ratio_GPP_Reco)
...
# Assess model performance
Ratio_GPP_Reco<- Ratio_GPP_Reco[c("Site_ID",  "Type_Flux", "Annual_Preci", "Tair", "Rg", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "values")]
Ratio_GPP_Reco$prediction <- NA
for(id in unique(Ratio_GPP_Reco$Site_ID)){
  train.df <- Ratio_GPP_Reco[Ratio_GPP_Reco$Site_ID != id,]
  test.df <- Ratio_GPP_Reco[Ratio_GPP_Reco$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP")]
  lm.Ratio_GPP_Reco<- lm(values ~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair, data=train.df)
...
# Stepwise regression
lm.Ratio_GPP_Reco_Mean_Site<-lm(values ~ (f_P + f_Tair + f_Age +f_GPP)^2, data=Ratio_GPP_Reco_Mean_Site)
...
# Assess model performance
Ratio_GPP_Reco_Mean_Site<- Ratio_GPP_Reco_Mean_Site[c("Site_ID",  "Type_Flux", "Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "values")]
...
for(id in unique(Ratio_GPP_Reco_Mean_Site$Site_ID)){
  ...
  test.df <- Ratio_GPP_Reco_Mean_Site[Ratio_GPP_Reco_Mean_Site$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP")]
  lm.Ratio_GPP_Reco_Mean_Site<- lm(values ~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair, data=train.df)
...
# Stepwise regression
lm.Ratio_NEP_GPP<-lm(values ~ (f_P + f_Tair + f_Age + f_GPP)^2, data=Ratio_NEP_GPP)
step.Ratio_NEP_GPP<- stepAIC(lm.Ratio_NEP_GPP, direction = "backward")
print(step.Ratio_NEP_GPP)
summary(step.Ratio_NEP_GPP)

# Compute Importance variable
bootswiss <- boot.relimp(values~  f_P + f_Tair + f_Age + f_GPP + f_P:f_Age + 
                           f_P:f_GPP + f_Tair:f_Age + f_Tair:f_GPP,
                         data= Ratio_NEP_GPP, 
                         b = 100,  
                         type = "lmg",
                         rank = TRUE, diff = TRUE, rela = T)
...
# Assess model performance
Ratio_NEP_GPP<- Ratio_NEP_GPP[c("Site_ID",  "Type_Flux", "Annual_Preci", "Tair", "Rg", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "values")]
Ratio_NEP_GPP$prediction <- NA
for(id in unique(Ratio_NEP_GPP$Site_ID)){
  train.df <- Ratio_NEP_GPP[Ratio_NEP_GPP$Site_ID != id,]
  test.df <- Ratio_NEP_GPP[Ratio_NEP_GPP$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP")]
  lm.Ratio_NEP_GPP<- lm(values ~   f_P + f_Tair + f_Age + f_GPP + f_P:f_Age + 
                          f_P:f_GPP + f_Tair:f_Age + f_Tair:f_GPP, data=train.df)
...
# Stepwise regression
lm.Ratio_NEP_GPP_Mean_Site<-lm(values ~ (f_P + f_Tair + f_Age + f_GPP)^2, data=Ratio_NEP_GPP_Mean_Site)
...
# Compute Importance variable
bootswiss <- boot.relimp(values~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair + 
                           f_P:f_Age + f_P:f_GPP + f_Tair:f_Age, 
...
# Assess model performance
Ratio_NEP_GPP_Mean_Site<- Ratio_NEP_GPP_Mean_Site[c("Site_ID",  "Type_Flux", "Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP", "values")]
...
for(id in unique(Ratio_NEP_GPP_Mean_Site$Site_ID)){
  ...
  test.df <- Ratio_NEP_GPP_Mean_Site[Ratio_NEP_GPP_Mean_Site$Site_ID == id, c("Annual_Preci", "Tair", "Stand_Age", "f_P", "f_Tair", "f_Age", "f_GPP")]
  lm.Ratio_NEP_GPP_Mean_Site<- lm(values ~ f_P + f_Tair + f_Age + f_GPP + f_P:f_Tair + 
                                    f_P:f_Age + f_P:f_GPP + f_Tair:f_Age, data=train.df)
